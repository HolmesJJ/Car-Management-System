...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	lock: function()
	{
		...
		that._lock = true;
		$('#ldg_lockmask').html('<div style=" height: 100%; left: 0px; position: absolute; top: 0px; width: 100%; opacity: 0.18;"></div>');	
		return that;
	},
...
(function(config){
    ...
    config['lock'] = true;
    config['fixed'] = true;
    config['padding'] = "0px 20px 10px 20px";
    config['min'] = false;
})($.dialog.setting);
...
//操作成功	
	ok:function(options){
		...
		$.extend(org, options);
		$.dialog({
			content: '<b class=font16Tag><i class=icoOk>&nbsp;</i>'+ org.con +'</b>',
			width:org.width,
			ok: function(){ org.okFun(org._this) }
		});
	},
//保存成功		
...
//是否删除
	del:function(options){
		var org = this.o;	
		org.width = 290;
		org.con = "删除后数据将不可恢复！<br />是否继续？"
		
		$.extend(org, options);
		$.dialog({
			okVal  : "继续",
			content: '<b class=font16Tag><i class=icoWarn>&nbsp;</i>'+ org.con +'</b>',
			width  : org.width,
			ok: function(){ org.okFun(org._this) },
			close: function(){ org.closeFun(org._this)},
			cancel:function(){}
		});
	},
//loading
	loading:function(options){
		...
		$.dialog({
			...
			title:false,
			max:false,
			content: '<span class=alertLoading><i>&nbsp;</i>'+ org.con +'</span>',
			width:170,
			height:60
		});
	}
